...
class Theme_My_Login_Template extends Theme_My_Login_Abstract {
	...
	public function get_title( $action = '' ) {
		...
		if ( is_user_logged_in() && 'login' == $action && $action == $this->get_option( 'default_action' ) ) {
			...
			if ( $page_id = Theme_My_Login::get_page_id( $action ) ) {
				...
				switch ( $action ) {
					case 'register':
						$title = __( 'Registrar', 'theme-my-login' );
						break;
					case 'lostpassword':
					case 'retrievepassword':
					case 'resetpass':
					case 'rp':
						$title = __( 'Recuperar contraseña', 'theme-my-login' );
						break;
					case 'login':
					default:
						$title = __( 'Ingresar', 'theme-my-login' );
				}
			}
		}
	...
	public static function get_action_template_message( $action = '' ) {
		switch ( $action ) {
			case 'register':
				$message = __( 'Registro para este Sitio', 'theme-my-login' );
				break;
			case 'lostpassword':
